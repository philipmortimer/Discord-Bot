...
const add = context.params.event.data.options.length === 2 ? 
  context.params.event.data.options[1].value : 1;
const role = context.params.event.data.options[0].value;
if (shared.isValidRole(role) && !points.error && add > 0){
  //Adds points and edits message
  points.points[role] = parseInt(points.points[role], 10) + add;
